...
function EdenCake:ResetItem(IsContinued)
    ...
    local ItemCount = ItemConfig:GetCollectibles().Size -1
    local ItemQuality = 0
...
function EdenCake:GetItem()
    ...
    if player:HasTrinket(TrinketType.TRINKET_BIRTHCAKE) and not hasTrinket then
        hasTrinket = true
...
function EdenCake:Undecided()
    ...
    if not EdenCake:CheckEdenB(player) or not player:HasTrinket(TrinketType.TRINKET_BIRTHCAKE) then
        return
    end
        ...
        if entity.Type == EntityType.ENTITY_PICKUP and entity.Variant == PickupVariant.PICKUP_COLLECTIBLE and entity.SpawnerType ~= EntityType.ENTITY_PLAYER then
            ...
            if roll < 0.5 then
                ...
                if roll < 0.5 then
                    ...
                    entity:Remove()
                    local newEntity = Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, -1, entity.Position, Vector(0, 0), player):ToPickup()
