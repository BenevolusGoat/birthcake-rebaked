...
function TheLostCake:FreeUse(collectibleID, rngObj, player, useFlags, activeSlot, varData)
    if not TheLostCake:CheckLost(player) or not player:HasTrinket(TrinketType.TRINKET_BIRTHCAKE) then
        return nil
    end
...
function TheLostCake:Fortune()
    ...
    if not TheLostCake:CheckLostB(player) or not player:HasTrinket(TrinketType.TRINKET_BIRTHCAKE) then
        return
    end
    ...
    for i = 1, #entites do
        ...
        if entity.Type == EntityType.ENTITY_PICKUP and entity.Variant == PickupVariant.PICKUP_COLLECTIBLE and entity.SpawnerType ~= EntityType.ENTITY_PLAYER then
            ...
            if float < 0.5 then
                ...
                Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, -1, Isaac.GetFreeNearPosition(entity.Position,10), Vector(0,0), player)
                local float = rng:RandomFloat()
                if float < 0.25 then
                    Isaac.Spawn(EntityType.ENTITY_EFFECT,EffectVariant.BOMB_EXPLOSION, 0, Isaac.GetFreeNearPosition(player.Position, 10), Vector(0,0), player)
                    for j = 1, #entites do
