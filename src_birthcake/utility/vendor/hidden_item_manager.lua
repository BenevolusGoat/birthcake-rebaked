...
function HiddenItemManager:PostPlayerInit()
	local numPlayers = #Isaac.FindByType(EntityType.ENTITY_PLAYER)
	...
	for key, data in pairs(INDEX) do
		...
		if data.Initialized and not wisp and IsActivePlayer(player) then
			if data.ErrorCount >= 10 then
				...
				LOG_ERROR(
					"Something is constantly removing the Item Wisps or preventing them from spawning! Giving up on item #" ..
					data.Item .. " from group: " .. data.Group)
				RemoveWisp(key)
			else
				if data.ErrorCount == 0 then
					LOG_ERROR("Wisp disappeared unexpectedly! Respawning wisp for item #" ..
						data.Item .. " from group: " .. data.Group)
				end
...
AddCallback(ModCallbacks.MC_PRE_USE_ITEM, HiddenItemManager.StartSacrificialAltarProtection,
	...
	for key, data in pairs(INDEX) do
		...
		if wisp then
			...
			if player then
				...
				LOG_ERROR("Somehow lost track of player during Sacrificial Altar protection. Giving up on item #" ..
					data.Item .. " from group: " .. data.Group)
				if wisp then
...
AddCallback(ModCallbacks.MC_USE_ITEM, HiddenItemManager.FinishSacrificialAltarProtection,
	CollectibleType.COLLECTIBLE_SACRIFICIAL_ALTAR)
